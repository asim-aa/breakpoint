...
| — | GitHub Action: verifies an existing PR's code (single file or auto-detected diff), not just generated code | ✅ done — run live 3x, found and fixed a real crash, see below |
...
**This has now run live**, triggered against this repo's own `nodes/validator.py`, and it immediately found a real crash: the Skeptic model burned its entire token budget on internal reasoning before emitting the test array, and `find_bugs()` had no retry path for that failure mode — an unhandled `RuntimeError` took the whole Action down. Fixed (`find_bugs` now requests a wider token budget and retries on that specific error; `action/verify_pr.py`'s per-file loop now catches provider errors per file instead of crashing the whole run — see commit history). Re-triggered twice more afterward: both runs hit a second, unrelated live issue — a sustained 502 "Service temporarily overloaded" from Nvidia's free-tier Prover endpoint (the same outage that delayed the eval above) — and both times the fix did its job: a clean `⚠️ Skipped — provider error` report and a green run, instead of a crash. That's the resilience pattern proven live, twice, against a real and currently-flaky provider, even though Nvidia's instability has so far prevented seeing a full clean pass (spec inferred, tests generated, verdict reached) in this Action specifically. The diff-detection logic (`action/diff_utils.py`) was already proven separately against real git repos and this project's own commit history.
