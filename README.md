...
Once Nvidia's endpoint cooperated, it got a full clean run: triggered against [`demo/buggy_binary_search.py`](demo/buggy_binary_search.py), a real, deliberately-planted bug (`while lo < hi` instead of `lo <= hi`, missing the case where the target sits exactly at the final `lo == hi` position) with no hint left in the file itself. The live Skeptic inferred the spec, wrote adversarial tests blind, ran them for real, and **caught 4 real failures** — `test_single_element_found`, `test_target_at_first_index`, `test_target_at_last_index`, `test_two_elements_second` — exactly the boundary cases the bug actually breaks. That file is still in the repo; run the Action against it yourself to see the same result. The diff-detection logic (`action/diff_utils.py`) was already proven separately against real git repos and this project's own commit history.
